...
export interface ArduinoCodeModificationParamsFile {
  originalFilePath: string
  jsonFilePath: string
}

export interface ArduinoCodeModificationParamsRawJson {
  originalFilePath: string
  json: string
}
...
export async function modifyArduinoCodeWithImagesFromFile(
  params: ArduinoCodeModificationParamsFile
) {
  try {
      ...
      return images
        .map((image) => {
          const imageContent = image
            .map((value) => `B${value.toString(2).padStart(8, '0')}`)
            .join(', ')
          return `{ ${imageContent} }`
        })
        .join(',\n')
...
export async function modifyArduinoCodeWithImagesFromRawCode(
  params: ArduinoCodeModificationParamsRawJson
) {
  try {
    const { originalFilePath, json } = params
